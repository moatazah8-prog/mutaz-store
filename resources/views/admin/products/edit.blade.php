...
<title>تعديل المنتج - معتز ستور</title>
<style>
...
.current-image{width:100%;height:220px;background:#f5f5f5;border-radius:12px;display:flex;align-items:center;justify-content:center;overflow:hidden;margin-bottom:10px}
.current-image img{width:100%;height:100%;object-fit:contain}
</style>
</head>
<body>
<div class="container">
...
<option value="YER" {{ $product->currency == 'YER' ? 'selected' : '' }}>ريال يمني</option>
<option value="USD" {{ $product->currency == 'USD' ? 'selected' : '' }}>دولار</option>
</select>
...
<input type="checkbox" name="featured" value="1" style="width:auto" {{ $product->featured ? 'checked' : '' }}>
منتج مميز
</label>
